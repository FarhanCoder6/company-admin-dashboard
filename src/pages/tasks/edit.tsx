...
import { UPDATE_TASK_MUTATION } from "@/component/graphql/mutations";
...
const TasksEditPage = () => {
  ...
  // formProps -> Form instance and props
  const { modalProps, close, formProps, id } = useModalForm<Task>({
  ...
  // Fetch the task data using useOne hook with the id from useModalForm
  const { data, isLoading } = useOne<Task>({
    resource: "tasks",
    id: id ?? "",
    queryOptions: {
      enabled: !!id,
  ...
  // get the data of the task from the query result
